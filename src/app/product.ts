...
export class ProductService {
  ...
  removeFromCart(productId: number) {
    const productTitle = this.cart().find((item) => item.productId === productId)?.title;
    this.cart.set(this.cart().filter((item) => item.productId !== productId));
    console.log(`Removed from cart: ${productTitle}`);
    console.log('Current cart:', this.cart());
  }
  ...
  removeFromFavorites(productId: number) {
    const productTitle = this.favorites().find((fav) => fav.id === productId)?.title;
